...
import { faCalendarCheck,  faSearch, faEye} from '@fortawesome/free-solid-svg-icons'

import Lottie from 'lottie-react-web';
import animation from '../../../animation/drawkit-grape-animation-5-LOOP.json';
...
export default class TicketsFinalizados extends Component {
    ...
    pegaTickets = async(idCliente)=>{
        await api.get('/suporte/'+idCliente).then(response=>{
            var Finalizado = response.data.filter(data => data.status === 'Resolvido')
            this.setState({
                data:Finalizado,
            })
        ...
        return (
            <div style={{marginTop:'66px', display:'flex'}}>
                ...
                <Container className="corpo">
                   <div className="TituloTopoCorpo">Tickets Finalizados</div>
                   <Row>
                       <Col md={8}>
                            <div className="cardMeusProjetos">
                                <div style={{display:'flex',justifyContent:'space-between', alignItems:'center'}}>
                                    <div><FontAwesomeIcon icon={faCalendarCheck} className={"iconeFormPRoj"}/></div>
                                    <div>
                                            ...
                                            renderResults={results => (
                                                ...
                                                {results.map(el => (
                                                    <ListGroup.Item key={el.id} >
                                                        <div style={{display:'flex',justifyContent:'space-between',alignItems:'center'}}>
                                                            <div>{el.nome}</div>
                                                            <div style={{marginLeft:'-17%'}}>%{el.progresso}</div>
                                                            <div>
